<template>
	<div class="ciname-list">
		<div>
			<header class="header">
			<div class="left">
				<div class="city" data-enter-time="1602496952" data-click-fun="track_f_798828">
					<span @click="location()">{{cityName}}</span>
					<img
						src="data:image/png;base64," width="6px" height="3px"
					/>
				</div>
			</div>
			<div class="title">
				<div>影院</div>
			</div>
			<div class="right">
				<div>
					<div
						class="toCinemaSearch" data-enter-time="1602496952" data-click-fun="track_f_960613"
					>
						<img
							src="data:image/png;base64," width="18px" height="18px" alt=""
						/>
					</div>
				</div>
			</div>
		</header>
		<div class="cinema-list-tag">
			<div class="cinema-list-tag-name">
				<label class="">
					全城
					<img
						src="data:image/png;base64,"
						width="6px"
						height="3px"
						style="display: none" />
					<img
						src="data:image/png;base64,"
						width="6px"
						height="3px"
				/></label>
				<label class="">
					APP订票
					<img
						src="data:image/png;base64,"
						width="6px"
						height="3px"
						style="display: none" />
					<img
						src="data:image/png;base64,"
						width="6px"
						height="3px"
				/></label>
				<label class="">
					最近去过
					<img
						src="data:image/png;base64," width="6px" height="3px" style="display: none" />
					<img
						src="data:image/png;base64," width="6px" height="3px"
				/></label>
			</div>
		</div>
		 <loading v-if="loading"></loading>
            <div class="cinema-layer"></div>
			<div class="scroll">
				<div class="cinema-banner-wrap">
					<ul class="cinema-list">
						<li
							class="cinema-banner-item" v-for="items in Gateway" :key="items.bannerId">
							<div class="name">
								<span class="title">{{ items.name }}</span>
								<span class="icon">影院券</span>
							</div>
							<div class="type">{{ items.masterTitle }}</div>
						</li>
					</ul>
				</div>
				<div class="cinema-list-wrap">
					<ul class="cinema-list">
						<li class="cinema-list-item" v-for="item in Cinema" :key="item.cinemaId">
							<a href="#/cinema/1000/film" class="cinema-item-wrap">
							<div class="cinema-info-lf cinema-info">
									<span class="cinema-name">{{ item.name}}</span>
									<img src="data:image/png;base64,"
									 alt="去过"	class="isVisted"	style="display: none"/>
									<span class="cinema-address">{{ item.address }}</span>
								</div>
								<div class="cinema-info-rt cinema-info">
									<div class="">
										<span class="cinema-lowPrice price-fmt" ><i v-if="item.lowPrice != 0">￥</i>
											<span class="interge" style="font-size:15px" v-if="item.lowPrice != 0">{{item.lowPrice/100}}</span>
											<span style="display: none">.</span>
											<span class="decimal" style="font-size: 10px"></span>
											<span style="display: none">.</span>
											
										</span>
										<span class="upon" v-if="item.lowPrice != 0"> 起</span>
									</div>
									<span class="cinema-gpsAddress" 
										><img
											src="data:image/png;base64,"
											alt="距离"
											style="display: none"
										/>
										<!-- {{item.Distance}} -->
										<strong>距离未知</strong></span>
								</div></a
							>
						</li>
					</ul>
				</div>
			</div>
		</div>
	</div>
</template>
<script>
import loading from '@/components/loading'
import { gatewayData, cinemaData } from "@/api/api"


export default {
	data() {
		return {
			loading:true,
			Cinema: [],
			Gateway: [],
			cityName:'',
			clickName :'',
			cityId:''
			
		};
		
		
		
	},
	
	
    //  filters: {
    //     //  
    //     lowPrice: function(value) {
         
    //         if (value==0) {
    //             this.lowPrice=''
                  
    //             };
    //         }
           
    //     },
    //       lowPrice: function(value){
    //     // 时间戳的单位是秒，需要乘以1000转化成毫秒进行处理
    //     return lowPrice(value/100).format('ddd MM月DD日')
    //     },

	
	computed:{
		// ...mapState(['city'])
		
	},
	 components: {
        loading
    },
	
	methods: {
		 getLocation() {
            let _this = this;
            AMap.plugin("AMap.Geolocation", function() {
                var geolocation = new AMap.Geolocation({
                    // 是否使用高精度定位，默认：true
                    enableHighAccuracy: true,
                    // 设置定位超时时间，默认：无穷大
                    timeout: 5000,
                });
                geolocation.getCurrentPosition();
                AMap.event.addListener(geolocation, "complete", onComplete);
                AMap.event.addListener(geolocation, "error", onError);
                // data是具体的定位信息
                function onComplete(data) {
// 						// 城市区号
						console.log(data);
                    let cityId = data.addressComponent.adcode.substr(0,4)*100; 
				
						_this.cityId=cityId  
						
						localStorage.setItem("cityId",cityId)
    					// 城市名字(取2位)不带市--city市-- province省
						let cityName1 = data.addressComponent.province
						let cityIndex = cityName1.indexOf('市')
						let city1 =cityName1.substr(0,cityIndex)
						
                    	let cityName2 = data.addressComponent.province.substr(0,2)
						if(cityName1 == ''){
							_this.cityName = cityName2
						}else{
							_this.cityName =city1
						}
					
					
					
// 						// 点击城市的名字
					
//                     let clickName = localStorage.getItem("clickName");
						
//                     _this.getCityId(cityId);
						
					
						
//                     if(cityName != ''){
						localStorage.setItem("cityName", _this.cityName);
						// }
//                     _this.getCityName(cityName);
//                     if (clickName == "undefined" || clickName == null) {
//                         _this.city1 = cityName;
//                     } else {
//                         _this.city1 = clickName;
//                     }
                }
					
                function onError(data) {
                    // 失败 启用 ip定位
                    AMap.plugin("AMap.CitySearch", function() {
                        var citySearch = new AMap.CitySearch();
                        citySearch.getLocalCity(function(status, result) {
						// console.log(citySearch.getLocalCity);
                            if (status === "complete" && result.info === "OK") {
                                // 查询成功，result即为当前所在城市信息
                                console.log("通过ip获取当前城市：", result);

                            }
                        });
                        return;
                    });
                }
            });
        },
	
	location:function(){
		this.$router.push({path:'/city'})
		} 
	},
	created() {
		if(localStorage.getItem('cityName') == null){
			this.getLocation();
		}	
	},

	async mounted() {
		let city=localStorage.getItem('clickId')
		if(city==null){
			city=310100
		}
		
		var _this = this;	//310100
		var ret = await cinemaData(city);
		this.Cinema = ret.data.data.cinemas;
		var res = await gatewayData(city);
		
		// this.data = this.data.concat(ret.data.data.films)	
		this.Gateway = res.data.data;
		let clickName = localStorage.getItem('clickName')
					
						if(clickName != null && clickName != 'undefined'){
							
							this.cityName=clickName
					
						}else{
							this.cityName = localStorage.getItem('cityName')
						}
	
	if(this.Cinema.length > 0|| this.Gateway.length>0){
            this.loading = false
		}
			

}
}
</script>
<style lang="scss" scoped>
header {
	position: fixed;
	top: 0;
	left: 0;
	width: 100%;
	height: 44px;
	line-height: 44px;
	overflow: hidden;
	margin: 0;
	padding: 0;
	z-index: 3000;
	background-color: #fff;
	display: -webkit-box;
	display: -ms-flexbox;
	display: flex;

	.left {
		min-width: 15%;
		display: -webkit-box;
		display: -ms-flexbox;
		display: flex;
		-webkit-box-pack: start;
		-ms-flex-pack: start;
		justify-content: flex-start;
		-webkit-box-align: center;
		-ms-flex-align: center;
		align-items: center;
		margin-left: 15px;
		.city {
			display: -webkit-box;
			display: -ms-flexbox;
			display: flex;
			-webkit-box-align: center;
			-ms-flex-align: center;
			align-items: center;
		}
		.city span {
			display: inline-block;
			font-size: 13px;
			max-width: 58px;
			height: 16px;
			line-height: 16px;
			overflow: hidden;
			-o-text-overflow: ellipsis;
			text-overflow: ellipsis;
			white-space: nowrap;
			padding-right: 5px;
			color: #191a1b;
		}
	}
	.title {
		text-align: center;
		font-size: 17px;
		color: #191a1b;
		-webkit-box-flex: 1;
		-ms-flex: 1;
		flex: 1;
		overflow: hidden;
		-o-text-overflow: ellipsis;
		text-overflow: ellipsis;
		white-space: nowrap;
	}
	.right {
		width: 15%;
		display: -webkit-box;
		display: -ms-flexbox;
		display: flex;
		-webkit-box-pack: center;
		-ms-flex-pack: center;
		justify-content: center;
		-webkit-box-align: center;
		-ms-flex-align: center;
		align-items: center;
		margin-right: 15px;
		div {
			width: 100%;
			text-align: right;
		}
	}
}
header > div {
	float: left;
	height: 44px;
}
header img {
	vertical-align: middle;
}
.cinema-list-tag {
	position: fixed;
	height: 50px;
	width: 100%;
	top: 44px;
	right: 0;
	text-align: center;
	background-color: #fff;
	z-index: 2000;
	.cinema-list-tag-name {
		position: relative;
		label {
			float: left;
			width: 33.3%;
			line-height: 49px;
			font-size: 14px;
			color: #191a1b;
			letter-spacing: -0.2px;
			img {
				vertical-align: middle;
				padding-left: 2px;
			}
		}
	}
}
.cinema-layer {
	padding-top: 94px;

	
}
.scroll{
		overflow-x: hidden;
	}

.cinema-banner-wrap {
	border-top: 1px solid #888888;
	list-style: none;
	padding: 0;
	margin: 0;
	.cinema-banner-item {
		padding: 15px;
		position: relative;
		background-color: #fff;
		.name {
			font-size: 15px;
			color: #18191a;
			position: relative;
			.icon {
				color: #ffb245;
				margin-left: 7px;
				font-size: 10px;
				padding: 0 6px;
				text-align: center;
				position: absolute;
				top: 50%;
				-webkit-transform: translateY(-50%);
				-ms-transform: translateY(-50%);
				transform: translateY(-50%);
			}
			.icon:after {
				content: " ";
				-webkit-transform: scale(0.5);
				-ms-transform: scale(0.5);
				transform: scale(0.5);
				position: absolute;
				border: 1px solid #ffb245;
				top: -50%;
				right: -50%;
				bottom: -50%;
				left: -50%;
				border-radius: 4px;
			}
		}
		.type {
			margin-top: 8px;
			font-size: 13px;
			color: #797d82;
		}
	}
}
.cinema-list-wrap {
	text-decoration:none;
	padding-bottom: 49px;
	z-index: 100;
	ul {
		list-style: none;
		padding: 0;
		margin: 0;
	}
	li {
		border-top: 1px solid #888888;

		position: relative;
		background-color: #fff;
		padding: 17px;
		.cinema-item-wrap {
			width: 100%;
			display: -webkit-box;
			display: -ms-flexbox;
			display: flex;
			-webkit-box-pack: start;
			-ms-flex-pack: start;
			justify-content: flex-start;
			-webkit-box-align: center;
			-ms-flex-align: center;
			align-items: center;

			.cinema-info-lf {
				width: calc(100% - 65px);
				text-align: left;
				padding-right: 15px;
				float: left;
				.cinema-name {
					margin: -2px;
					color: #191a1b;
					font-size: 15px;
					float: left;
				}
				span {
					display: block;
					max-width: 80%;
					overflow: hidden;
					-o-text-overflow: ellipsis;
					text-overflow: ellipsis;
					white-space: nowrap;
					float: left;
				}
				.cinema-address {
					color: #797d82;
					font-size: 12px;
					margin-top: 5px;
				}
			}
			.cinema-info-rt {
				
				width: 70px;
				text-align: center;
				float: right;
				margin-right: -5px;
				.price-fmt {
					font-size: 15px;
					color: #ff5f16;
					height: 0 !important;
					display: inline-table !important;
				}
				.price-fmt i {
					margin: -2px;
					font-size: 11px;
					font-style: normal;
				}
				.upon {
					color: #ff5f16;
					font-size: 10px;
				}
				.cinema-gpsAddress {
					display: block;
					font-size: 11px;
					color: #797d82;
					margin-top: 5px;
				}
				img {
					display: block;
					width: 13px;
					height: 17.5px;
					margin: 0 auto;
					margin-top: -30px;
				}
				strong {
					font-weight: 400;
					display: block;
					margin-top: 5px;
				}
			}
		}
	}
}
</style>